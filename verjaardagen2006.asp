...
sqlString = "SELECT naam, voornaam, geboortedatum, foto, tblfct1.functie AS fct1, tblfct2.functie AS fct2, " &_
			...
			"WHERE status = 'A' AND (DAYOFYEAR( geboortedatum ) - DAYOFYEAR( CURRENT_DATE( ) ) < 8 ) " &_
			"AND (DAYOFYEAR( geboortedatum ) - DAYOFYEAR( CURRENT_DATE( ) ) ) > -1 " &_ 
  ...
  while not rs.eof
   ...
   <tr><td nowrap style="border-bottom: 1px solid #000099;"><blockquote><p>
	...
	<%end if%>
			...
			if rs("functie1") = 5 or rs("functie2") = 5 then response.Write("Scheidsrechter <br />")			
			%></i></p>
	      </blockquote></td>
	  </tr>
	<%
