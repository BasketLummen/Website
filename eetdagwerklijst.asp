...
{
	TheForm.betalen.value = (eval(TheForm.mosselen.value)*15)+((eval(TheForm.vide.value)+eval(TheForm.halvehaan.value)+eval(TheForm.stoofvlees.value))*8)+(eval(TheForm.hamburgers.value)*5);
}
</script>
<style>
.csstotaal {
...
<div id="Layer3" style="position:absolute; width:719px; height:436px; z-index:1; left: 120px; top: 70px;">
  ...
  <p class="NieuwsTitels style1">Werklijst eetdag 2016</p>
  <p>In onderstaande tabel vind je het werkplan voor het eetdag van Basket Lummen van zondag 23 oktober.<br>
  ...
  <p><SPAN XSSCleaned="font-size:10.0pt; font-family:Arial">Iedereen wordt een kwartiertje voor de start van de shift verwacht.</SPAN></p>
  <p><SPAN XSSCleaned="font-size:10.0pt; font-family:Arial">Degenen die vooraf met de   ploeg nog willen eten, moeten dit wel tijdig doen zodat ze klaar zijn om te   werken op het vermelde uur ( dus niet om 16.30 uur eerst gaan eten tot 17.00 uur   als je verwacht wordt beginnen te werken om 16.30 uur ). (Eventueel samen met je   ploeg eten na je werkshift ).</SPAN></p>
...
sqlstring = "SELECT * FROM  tbleetdagtaken ORDER BY id"
...
while not rsTaken.eof
    ...
    <%for i = 1 to 2%>
        ...
        <%sqlstring = "SELECT naam FROM tbleetdagtaakverdeling WHERE taaknr = " & rsTaken("id") & " AND shift = " & i
		rs.open sqlstring
		tel = 0
		while not rs.eof
			tel = tel + 1
