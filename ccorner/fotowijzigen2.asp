...
load.initialize   'calling initialize method
	...
	rs.CursorLocation = 3
	sqlstring = "SELECT ploegfoto FROM tblploegen WHERE ploegid = " & id 
	rs.open sqlstring, con, 2, 3
	if not isnull(rs("ploegfoto")) and rs("ploegfoto") <> "" then
		sqlString = "DELETE FROM tblfotos WHERE fotoid = " & rs("ploegfoto")
	...
	if fileSize > 0 then
		...
		rs.open sqlString
		sqlstring = "UPDATE tblploegen SET ploegfoto = " & rs("fotoid") & " WHERE ploegid = " & id
		con.execute sqlstring
		rs.close
	end if
	
con.close
response.Redirect("ploegen.asp?ploegid="&id)%>
</div>
</body>
</html>
