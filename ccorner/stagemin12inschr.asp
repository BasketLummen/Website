...
<%
sqlstring = "SELECT * FROM tblmin12stage ORDER BY id"
rs.open sqlstring
%>
	<p class="NieuwsTitels"><font size="3">Inschrijvingen Stage</font></p>
    ...
    <%while not rs.eof%>
    	<tr>
         <td valign="top"><%=rs("naam")%></td>
            ...
            <td valign="top"><%if(rs("dinsdag")="on") then response.Write("X")%></td>            
            <td valign="top"><%if(rs("woensdag")="on") then response.Write("X")%></td>            
             <td valign="top"><%if(rs("donderdag")="on") then response.Write("X")%></td>
   	</tr>
    	<%
		rs.movenext
	wend
